...
// Параметры:
//   НовоеЗначение - Число, Null - Новое значение счетчика. Если Null, возвращается текущее значение.
//   - Если параметр Null, метод работает как геттер и возвращает текущее значение (Число).
//   - Если указан параметр, метод устанавливает новое значение (должно быть числом) и возвращает ЭтотОбъект для цепочки вызовов.
// Возвращаемое значение:
//   Число - Текущее значение счетчика (если Null).
//   ЭтотОбъект - Для цепочки вызовов (если указано значение).
Функция Значение(НовоеЗначение = Null) Экспорт
...
// Параметры:
//   НовоеЗначение - Число, Null - Если указано, устанавливает новое значение (должно быть числом).
//   - Если параметр не указан (или Null), метод работает как геттер и возвращает текущее значение.
//   - Если указан параметр, метод устанавливает новое значение и возвращает ЭтотОбъект для цепочки вызовов.
// Возвращаемое значение:
//   Число - Текущее значение минимум (если без параметра).
//   ЭтотОбъект - Для цепочки вызовов (если указано значение).
Функция Минимум(НовоеЗначение = Null) Экспорт
    Если НовоеЗначение = Null Тогда
        Возврат Минимум;
    КонецЕсли;
    Если ТипЗнч(НовоеЗначение) <> Тип("Число") Тогда
...
// Параметры:
//   НовоеЗначение - Число, Null - Новое максимальное значение. Если Null, возвращается текущее значение.
//   - Если параметр Null, метод работает как геттер и возвращает текущее значение (Число или Неопределённо).
//   - Если указан параметр, метод устанавливает новое значение и возвращает ЭтотОбъект для цепочки вызовов.
// Возвращаемое значение:
//   Число, Неопределённо - Текущее значение Максимум (если Null).
//   ЭтотОбъект - Для цепочки вызовов (если указано значение).
Функция Максимум(НовоеЗначение = Null) Экспорт
    Если НовоеЗначение = Null Тогда
        Возврат Максимум;
    КонецЕсли;
...
Функция Описание(НовоеЗначение = Null) Экспорт
    Если НовоеЗначение = Null Тогда
        Возврат Описание;
    КонецЕсли;
...
Функция ИнтервалОбновления(НовоеЗначение = Null) Экспорт
    Если НовоеЗначение = Null Тогда
...
Процедура Добавить(Счет = 1) Экспорт
	...
	Инициализация(); // для оценки превышения
КонецПроцедуры
...
Функция ПрогрессСтрокой() Экспорт
    Возврат СокрЛП(Строкой() + ?(Максимум = Неопределено ,"", " Выполнено: " + ПроцентВыполнения() + "%"));
КонецФункции
...
Функция Строкой(ПоказыватьИтог = Истина, ПоказыватьСчет = Истина, ПоказыватьОстаток = Истина) Экспорт
    ...
    М = Новый Массив;
    ФорматЧисла = "ЧН=; ЧГ=";
	Если ПоказыватьИтог И Максимум <> Неопределено Тогда
        М.Добавить(Формат(Максимум, ФорматЧисла));
    КонецЕсли;
    Если ПоказыватьСчет Тогда
        М.Добавить(Формат(Значение, ФорматЧисла));
    КонецЕсли;
    Если ПоказыватьОстаток И Максимум <> Неопределено Тогда
        М.Добавить(Формат(Максимум - Значение, ФорматЧисла));
    КонецЕсли;
...
// Параметры:
// Ошибки - Массив  - масив содержит описание ошибок 
...
// Параметры:
// Ошибки - Массив  - массив содержит описание ошибок 
Процедура Тест2(Ошибки)
    Счетчик = Обработки.ws64_Счетчик.Создать()
        ...
        .Описание("Тестовый счетчик")
        .Инициализация();
    Если Счетчик.Строкой() <> "100:0:100 Тестовый счетчик" Тогда
...
Процедура Тест8(Ошибки)
    ...
    Если Счетчик.Значение() <> 100 Или Не Счетчик.ВДиапазоне() Тогда
...
Процедура Тест9(Ошибки)
    Счетчик = Обработки.ws64_Счетчик.Создать()
        ...
        .Инициализация();
    Счетчик.Сброс();
    Если Счетчик.Значение() <> 0 Или НЕ Счетчик.ВДиапазоне() Тогда
...
Процедура Тест11(Ошибки)
    ...
    Если Счетчик.ПроцентВыполнения() <> 0 
        Или Счетчик.ПрогрессСтрокой() <> "100 Счетчик без максимума" 
        Тогда
...
Процедура Тест14(Ошибки)
    ...
    Счетчик = Обработки.ws64_Счетчик.Создать()
        ...
        .Описание(ДатаСеанса)
        .Инициализация();
    Если Счетчик.Описание() <> Строка(ДатаСеанса) Тогда
        Ошибки.Добавить("Тест 14: Ошибка преобразования даты в описание");
    КонецЕсли;
КонецПроцедуры

// Тест 15: Проверка установки максимума в Неопределённо
// Параметры:
//   Ошибки - Массив - массив содержит описание ошибок 
Процедура Тест15(Ошибки)
    Счетчик = Обработки.ws64_Счетчик.Создать()
        .Минимум(0)
        .Максимум(Неопределено)
        .ИнтервалОбновления(25)
        .Описание("Счетчик с установкой максимума в Неопределённо")
        .Инициализация();
    Если Счетчик.Максимум() <> Неопределено Тогда
        Ошибки.Добавить("Тест 15: Ошибка установки максимума в Неопределённо");
    КонецЕсли;
КонецПроцедуры

// Тест 16: Проверка установки отрицательного интервала обновления
// Параметры:
//   Ошибки - Массив - массив содержит описание ошибок 
Процедура Тест16(Ошибки)
    Счетчик = Обработки.ws64_Счетчик.Создать()
        .Минимум(0)
        .Максимум(100)
        .ИнтервалОбновления(-10)
        .Описание("Счетчик с отрицательным интервалом")
        .Инициализация();
    Если Счетчик.ИнтервалОбновления() <> 1 Тогда
        Ошибки.Добавить("Тест 16: Ошибка обработки отрицательного интервала обновления");
    КонецЕсли;
КонецПроцедуры

// Тест 17: Проверка установки Неопределённо для Значение
// Параметры:
//   Ошибки - Массив - массив содержит описание ошибок 
Процедура Тест17(Ошибки)
    Счетчик = Обработки.ws64_Счетчик.Создать();
    Попытка
        Счетчик.Значение(Неопределено);
        Ошибки.Добавить("Тест 17: Ошибка, не вызвано исключение при установке Неопределённо для Значение");
    Исключение
        Возврат;
    КонецПопытки;
КонецПроцедуры
