...
Функция УникальныйФайл(Каталог,Префикс="",Расширение="tmp") Экспорт
	
	Рсш = "."+СтрЗаменить(Расширение,".","");
	СЧ = 0; 
	Ф = Новый Файл(Каталог+"\"+Префикс+Фрм.ФЧС(СЧ)+Рсш);
	Пока Ф.Существует() Цикл
		СЧ = СЧ + 1;
		Ф = Новый Файл(Каталог+"\"+Префикс+Фрм.ФЧС(СЧ)+Рсш);
	КонецЦикла;   
	Возврат Ф;
