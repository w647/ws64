...
Функция Логгер(НовыйЛоггер = Null) Экспорт
    ...
    Если ТипЗнч(НовыйЛоггер) <> Тип("ОбработкаОбъект.ws64_Логгер") Тогда
        ВызватьИсключение Исключения.ЗапрещенныйЛоггер;
    КонецЕсли;
    Логгер = НовыйЛоггер;
    Возврат ЭтотОбъект;
КонецФункции
...
Процедура ЗначенияПоУмолчаню()
    ...
    Исключения.Вставить("ЗапрещенныйЛоггер", НСтр("ru = 'Ожидается логгер типа ws64_Логгер.'"));
...
Процедура ПримерИспользования() Экспорт
	...
	Попытка
		Монитор.Статус(Символ(31) + "НедопустимыйКлюч", "Тест");
	Исключение
