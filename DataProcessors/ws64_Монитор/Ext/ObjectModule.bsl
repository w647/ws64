...
Функция Счетчик(Ключ, МинимальноеЗначение=Неопределено, МаксимальноеЗначение=Неопределено, ИнтервалОбновления=1000, Описание="") Экспорт
...
Процедура Очистить(Ключ=Неопределено, Тип="") Экспорт
	...
	Если Тип = "Статус" Или ПустаяСтрока(Тип) Тогда
		Если Статус.Свойство(Ключ) Тогда
			Статус.Удалить(Ключ);
			Индекс = Порядок.Найти("Статус." + Ключ);
			Если Индекс <> Неопределено Тогда
				Порядок.Удалить(Индекс);
			КонецЕсли;
		КонецЕсли;
