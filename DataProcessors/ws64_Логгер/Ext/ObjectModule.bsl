...
Функция УровеньЛогирования(НовоеЗначение = Null) Экспорт
    ...
    КонецЕсли;
    УровеньЛогирования = Цел(Мин(Макс(НовоеЗначение, 1), 5));
    Возврат ЭтотОбъект;
КонецФункции
...
Функция Инициализация() Экспорт
    ...
    Если ВыводВФайл Тогда
        ...
        Если Каталог.Существует() И Каталог.ЭтоКаталог() Тогда
            ИмяФайлаЛога = Файл.Путь + Файл.ИмяБезРасширения + "." + Формат(ВремяСтарта, "ДФ=yyMMddHHmm") + Файл.Расширение;
...
Функция Ошибка(Текст, Объект = Undefined) Экспорт
    Если УровеньЛогирования < 1 Тогда
        Возврат ЭтотОбъект;
    КонецЕсли;
...
Функция Трасса(Текст, Объект = Undefined) Экспорт
    Если УровеньЛогирования < 5 Тогда
        Возврат ЭтотОбъект;
    КонецЕсли;
...
Процедура Лог(Текст, Объект = Undefined, Префикс = "", Статус = Undefined, УровеньЖурнала = Undefined)
    ...
    ФорматированноеСообщение = ФорматСообщения(Префикс, Текст);
    ...
    Если ВыводВЖурнал Тогда
        ...
        ЗаписьЖурналаРегистрации(Событие, УровеньЖурналаРегистрации, ?(Объект = Undefined, Undefined, Объект.Метаданные()), Объект, Текст, РежимТранзакцииЗаписиЖурналаРегистрации.Независимая);
    КонецЕсли;
КонецПроцедуры
...
Процедура УдалитьСтарыеФайлы()
    ...
    ПериодСекунд = ПериодХраненияЧасов * 60 * 60;
    Файл = Новый Файл(ПутьКФайлуЛога);
    ...
    Для Каждого НайденныйФайл Из Файлы Цикл
        Если НайденныйФайл.ЭтоКаталог() Тогда
            Продолжить;
        КонецЕсли;
        Если ТекущаяДата() - НайденныйФайл.ПолучитьВремяИзменения() > ПериодСекунд Тогда
...
Функция Таймер()
    ВремяПрошло = ТекущаяДата() - ВремяСтарта;
    Часы = Цел(ВремяПрошло / 3600);
    Минуты = Цел(ВремяПрошло % 3600 / 60);
    Секунды = ВремяПрошло % 3600 % 60;
    ФорматЧасов = "ЧДЦ=0; ЧН=; ЧГ=";
    ФорматМинутСекунд = "ЧЦ=2; ЧДЦ=0; ЧН=; ЧВН=";
    Возврат Формат(Часы, ФорматЧасов) + ":" + Формат(Минуты, ФорматМинутСекунд) + ":" + Формат(Секунды, ФорматМинутСекунд);
КонецФункции
...
Функция ФорматСообщения(Префикс = "", Текст)
    Возврат ""
        + ?(ПечатьДаты, Формат(ТекущаяДата(), "ДФ=HH.mm.ss") + " ", "")
...
Процедура Тест1(Ошибки)
    Логгер = Обработки.ws64_Логгер.Создать()
        ...
        .ВыводВЖурнал(Ложь)
        .Инициализация();
    Если Логгер.UровеньЛогирования() <> 2
...
Процедура Тест4(Ошибки)
    ...
    Если Не СтрНачинаетсяС(СообщенияЛога, Формат(ТекущаяДата(), "ДФ=HH.mm.ss"))
