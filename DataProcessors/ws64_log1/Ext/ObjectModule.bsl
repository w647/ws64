...
Функция ЗаписыватьВФайл(ПолноеИмяФайла) Экспорт
	    ФайлЛога = ПолноеИмяФайла;
		Возврат ЭтотОбъект;
КонецФункции
Функция ВывестиВТекстовыйДокумент(ПолноеИмяФайла) Экспорт
	
КонецФункции
Функция Мониторинг(Булево=Ложь) Экспорт
	Статус = Новый Структура;
	Возврат ЭтотОбъект;
КонецФункции
Функция ДобавитьСчетчик(Имя="Счет",Мин=0,Макс=0,Кратн=1)
	Лог.Счетчик.К.Добавить()
КонецФункции  
Функция УдалитьСчетчик(Имя="Счет",Мин=0,Макс=0,Кратн=1)
	Лог.Счетчик.К.Добавить()
КонецФункции  
...
Процедура СтрокаСостояния(Текст = "") Экспорт
	
	Если СтрокаСостояния Тогда
		Если ОбычноеПриложение Тогда
			стр = ?(ПечатьТаймер,Таймер(),"") + СокрЛП(Текст);
			Выполнить("Состояние(стр)");
		КонецЕсли;
	КонецЕсли
	
КонецПроцедуры
Процедура Статус() Экспорт
	
	Если НЕ (СтрокаСостояния 
		ИЛИ СтатусФайл) Тогда
		Возврат;
	КонецЕсли;
	
	Если Структура = Неопределено Тогда
		Структура = Новый Структура;
		Структура.Вставить("Время",Таймер());
	КонецЕсли;
	
	ДС = Новый ЗаписьJSON;
	ДС.УстановитьСтроку();
	ЗаписатьJSON(ДС,Структура);
	стр = ДС.Закрыть();
...
КонецПроцедуры    
Процедура НовыйСчетчик(Имя,Мин=0,Макс=0,Кратность=1)
     Лог.Стат.К.Добавить();
	 Лог.Стат.Запись();
КонецПроцедуры
